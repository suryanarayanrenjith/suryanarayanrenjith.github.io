...
(function () {
        ...
        p.draw = function () {
          ...
          for (let i = -halfCellsX; i <= halfCellsX; i++) {
            for (let j = -halfCellsY; j <= halfCellsY; j++) {
              ...
              let centerFront = p.createVector(
              ...
              let centerBack = p.createVector(
                (v4.x + v5.x + v6.x + v7.x) / 4,
  ...
  if (localStorage.getItem("forceFallback") === "true") {
    console.info("Force fallback mode is enabled based on previous performance.");
    runFallbackAnimation();
  } else {
    if (isWebGLAvailable()) {
      try {
  ...
  window.addEventListener("keydown", function (e) {
    if (e.key === "r" || e.key === "R") {
      console.info("Resetting fallback mode.");
      localStorage.removeItem("forceFallback");
      window.location.reload();
    }
  });
